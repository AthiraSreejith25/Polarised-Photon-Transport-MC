...
for s in range(len(detector.stokes_list)): #the detector stokes_list will be the same of the sources, hence iterating through detector.stokes_list
	...
	print ('------',polarisation[s]) #flag

	for i_photons in range(n_photons):

		print (i_photons/n_photons) #flag

		photon = Photon(solution,detector.stokes_list[s],wavelength,15*10**-6) #core_diam 1.2mm #150 micron
...
longest = max([len(i) for i in photon_trajectories_z[6]])

for i in range(longest):

	for j in range(len(photon_trajectories_z[6])):

		if i < len(photon_trajectories_z[6][j]):

			if i > 2:

				for alpha in range(1,4):

					pl.plot(photon_trajectories_z[6][j][i-4+alpha:i-2+alpha],photon_trajectories_y[6][j][i-4+alpha:i-2+alpha],'#1167b1{}'.format(str(hex(int(alpha*255/7)))[2:])) #blue

				pl.plot(photon_trajectories_z[6][j][i],photon_trajectories_y[6][j][i],color = '#03ac13{}'.format(str(hex(80))[2:]), marker = '.') #green

			else:

				pl.plot(photon_trajectories_z[6][j][:i+1],photon_trajectories_y[6][j][:i+1],'#1167b1{}'.format(str(hex(80))[2:])) #blue
				pl.plot(photon_trajectories_z[6][j][i],photon_trajectories_y[6][j][i],color = '#03ac13{}'.format(str(hex(80))[2:]), marker = '.') #green

		else:

			pl.plot(photon_trajectories_z[6][j],photon_trajectories_y[6][j],'#d21404{}'.format(str(hex(30))[2:])) #red
