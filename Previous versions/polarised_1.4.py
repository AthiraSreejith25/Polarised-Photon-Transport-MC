...
target = './run/{}'.now.strftime('%H:%M:%S_%d-%m')
...
'''
	#max length a photon travels before dying (we don't need that because of two reasons: 1) detector 2) we use weights
...
#		self.pos = [r*np.cos(theta),r*np.sin(theta),0] #do we need this, can't we use self.path_i[-1]?
		...
		self.dist = 0
#		self.time = 0 #what is this? we don't need this
		...
		self.pos_z = 0
#		self.l_max = medium.rand_l_max() #check this we don't need this
		self.drxn = [0,0,1] #u
...
class Detector:
	...
	def make_bins(self,spatial_data,num_of_bins,bin_range): #fit all mode or zoom mode?
...
times = [i*(10**-15) for i in range(110)] #list of times (0 to 10ns) #see how it moves to know more about the bin error
# ~.3 microns per femtosecond
...
#	s = 1 #polarisation (remove this)
#	print ('------',s) #flag

	for i_photons in range(n_photons):

		print (time_taken*(n_photons-i_photons)*(7-s))
#		print (i_photons/n_photons) #flag

		photon = Photon(glass,detector.stokes_list[s],wavelength,150*10**-6)
...
#				print ('reached detector @ ', photon.pos_x, photon.pos_y)
				detector.spatial_data[s].append((photon.pos_x,photon.pos_y)) #unblur the coordinates (hypermetropic detector) [DONE]
...
#				detector.polarization_data[s].append((photon.drxn,photon.ref))
		...
		time_taken = (default_timer()-start)/(s+1)

'''
for i in range(n_photons):

#	print (photon_trajectories_z[i][0],photon_trajectories_y[i][0])
	pl.plot (photon_trajectories_z[1][i],photon_trajectories_y[1][i])
...
for i in range(len(detector.stokes_list)):
	for j in range(len(detector.stokes_list)):
		...
		cls_intervals = detector.make_bins(detector.spatial_data[i],detector.reso,bin_range)
#		print (cls_intervals)
...
#			print (detector.polarization_data[i][k][j])
			pixel_array[int((spatial_data[k][0]+(bin_range/2))/bin_size)][int((spatial_data[k][1]+(bin_range/2))/bin_size)] += detector.polarization_data[i][k][j]
